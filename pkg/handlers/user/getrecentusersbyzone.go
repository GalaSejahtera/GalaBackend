...
type GetRecentUsersByZoneHandler struct {
	Model model.IModel
}

func (s *GetRecentUsersByZoneHandler) GetRecentUsersByZone(ctx context.Context, req *pb.GetUsersRequest) (*pb.GetUsersResponse, error) {

	users, err := s.Model.GetRecentUsersByZone(ctx, req.FilterValue)
	if err != nil {
		return nil, err
	}
...
func (s *GetRecentUsersByZoneHandler) usersToResponses(users []*dto.User) *pb.GetUsersResponse {
	var resps []*pb.User
	for _, user := range users {
		resp := &pb.User{
			Id:          user.ID,
...
func (s *GetRecentUsersByZoneHandler) sortUsers(users []*dto.User, field string, order string) {
	switch field {
	case "id":
